...
#define kYNPageScrollMenuViewConverMarginX 8

#define kYNPageScrollMenuViewConverMarginW 16
...
/// 自定义: 所有的item的背景蒙层
@property (strong, nonatomic) NSMutableArray *converViewArrayM;
...
+ (instancetype)pagescrollMenuViewWithFrame:(CGRect)frame
    ...
    menuView.converViewArrayM = @[].mutableCopy;
...
- (void)setupButton:(UIButton *)itemButton title:(NSString *)title idx:(NSInteger)idx {
    ...
    // Tag 值不建议配置 0。 这边没问题是因为: 不通过 viewwithTag 值取按钮, 都是通过下标取, tag的作用在于赋值 currentIndex
    itemButton.tag = idx;
    ...
    if (self.configration.showDefaultConver) {
        UIView *defaultConverView = [UIView new];
        defaultConverView.backgroundColor = self.configration.defaultConverColor;
        defaultConverView.layer.cornerRadius = self.configration.coverCornerRadius;
        defaultConverView.layer.masksToBounds = YES;
        [self.converViewArrayM addObject:defaultConverView];
        [self.scrollView addSubview:defaultConverView];
    }

    // 存储文字宽度
    [self.itemsWidthArraM addObject:@(itemButton.yn_width)];
    // 存储按钮, setupOtherViews 之后 按钮的实际宽度根据配置项被修改了
...
- (void)setupOtherViews {
    ...
    if (self.configration.showDefaultConver) {
        [self.converViewArrayM enumerateObjectsUsingBlock:^(UIView * _Nonnull defaultConverView, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx == 0) {
                converX += self.configration.itemLeftAndRightMargin;
            }else{
                converX += self.configration.itemMargin + [self.itemsWidthArraM[idx - 1] floatValue];
            }
            defaultConverView.frame = CGRectMake(converX - kYNPageScrollMenuViewConverMarginX, (self.scrollView.yn_height - self.configration.converHeight - self.configration.lineHeight) * 0.5, [self.itemsWidthArraM[idx] floatValue] + kYNPageScrollMenuViewConverMarginW, self.configration.converHeight);
        }];
    }
    ...
    if (scrollSizeWidht < self.scrollView.yn_width) {//不超出宽度
        ...
        if (self.configration.aligmentModeCenter && left >= 0) {
            ...
            if (!self.configration.scrollMenu) {
                ...
                if (self.configration.showDefaultConver) {
                    [self.converViewArrayM enumerateObjectsUsingBlock:^(UIView * defaultConverView, NSUInteger idx, BOOL * _Nonnull stop) {
                        itemW = [self.itemsWidthArraM[idx] floatValue] + kYNPageScrollMenuViewConverMarginW;
                        itemX = self.scrollView.yn_width/self.itemsArrayM.count*idx + (self.scrollView.yn_width/self.itemsArrayM.count - [self.itemsWidthArraM[idx] floatValue])/2 - kYNPageScrollMenuViewConverMarginX;
                        defaultConverView.frame = CGRectMake(itemX, (self.scrollView.yn_height - self.configration.converHeight - self.configration.lineHeight) * 0.5, itemW, self.configration.converHeight);
                    }];
                }
...
- (void)reloadView {
    ...
    [self.converViewArrayM removeAllObjects];
    [self setupSubViews];

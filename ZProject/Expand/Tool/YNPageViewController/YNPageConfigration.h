...
/** 是否显示默认遮盖*/
@property (nonatomic, assign) BOOL showDefaultConver;
/** 是否显示线条 YES */
...
/** 默认显示遮盖 NO */
@property (nonatomic, assign) BOOL showNormalConverColor;
/** 自定义Item 加图片 图片间隙 ... */
...
/** 默认遮盖color */
@property (nonatomic, strong) UIColor *defaultConverColor;
/** 默认遮盖颜色 */
@property (nonatomic, strong) UIColor *normalConverColor;
/** 菜单背景color */
...
/** 选项左右增加宽度 */
@property (nonatomic, assign) CGFloat itemAddWidth;
/** 选项字体 14 */
...
/** 其他高度(pageViewController作为一个子界面使用时调整scrollView的高度) */
@property (assign, nonatomic) CGFloat otherHeight;

/** custom: 分页的宽度 默认是 屏幕宽度 */
@property (assign, nonatomic) CGFloat pageViewWidth;
/** custom: 分页的高度 默认是 屏幕高度 */
@property (assign, nonatomic) CGFloat pageViewHeight;

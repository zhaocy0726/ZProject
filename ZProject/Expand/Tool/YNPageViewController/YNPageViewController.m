...
- (void)initViewControllerWithIndex:(NSInteger)index {
    ...
    // 内部判断, 当前scrollview上是不是已经有下个要展示的子视图View, 如果没有的话, 初始并添加
...
- (void)addViewControllerToParent:(UIViewController *)viewController index:(NSInteger)index {
    ...
    viewController.view.frame = CGRectMake(self.config.pageViewWidth * index, 0, self.pageScrollView.yn_width, self.pageScrollView.yn_height);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    CGFloat offsetX = currentPostion / self.config.pageViewWidth;
...
/// MARK: 调整scrollMenuView层级，防止TableView Section Header 挡住
...
- (void)updateViewWithIndex:(NSInteger)pageIndex {
    ...
    self.pageScrollView.contentSize = CGSizeMake(self.config.pageViewWidth * self.controllersM.count, self.pageScrollView.yn_height);
    
    UIViewController *vc = self.controllersM[pageIndex];
    
    vc.view.yn_x = self.config.pageViewWidth * pageIndex;
...
- (void)setupPageScrollView {
    ...
    CGFloat contentHeight = self.config.pageViewHeight - navHeight - tabHeight - cutOutHeight - self.config.otherHeight;
    
    if ([self isSuspensionTopPauseStyle]) {
        self.bgScrollView.frame = CGRectMake(0, 0, self.config.pageViewWidth, contentHeight);
        self.bgScrollView.contentSize = CGSizeMake(self.config.pageViewWidth, contentHeight + self.headerBgView.yn_height - self.config.suspenOffsetY);
        
        self.scrollMenuView.yn_y = self.headerBgView.yn_bottom;
        
        self.pageScrollView.frame = CGRectMake(0, self.scrollMenuView.yn_bottom, self.config.pageViewWidth, contentHeight - self.config.menuHeight  - self.config.suspenOffsetY);
        ...
        self.pageScrollView.frame = CGRectMake(0, [self isTopStyle] ? self.config.menuHeight : 0, self.config.pageViewWidth, ([self isTopStyle] ? contentHeight - self.config.menuHeight : contentHeight));
        
        self.pageScrollView.contentSize = CGSizeMake(self.config.pageViewWidth * self.controllersM.count, contentHeight - ([self isTopStyle] ? self.config.menuHeight : 0));
...
- (void)setupHeaderBgView {
    if ([self isSuspensionBottomStyle] || [self isSuspensionTopStyle] || [self isSuspensionTopPauseStyle]) {
        ...
        self.headerBgView.contentSize = CGSizeMake(self.config.pageViewWidth * 2, self.headerView.yn_height);
...
/// MARK: 将headerView 从 view 上 放置 tableview 上
...
/// MARK: 将headerView 从 tableview 上 放置 view 上
...
/// MARK: - 最终效果 current 拖到指定时 bg 才能下拉 ， bg 悬浮时 current 才能上拉
/// MARK: 计算悬浮顶部偏移量 - BgScrollView
...
/// MARK: 计算悬浮顶部偏移量 - CurrentScrollView
...
/// YNPageStyleFixation: HeaderView在顶部, MenuView 固定在 headerview 底部
- (BOOL)isFixationStyle {
    return self.config.pageStyle == YNPageStyleFixation ? YES : NO;
}
    ...
    if (self.config.headerViewCouldScale && self.scaleBackgroundView) {
        ...
        if (self.config.headerViewScaleMode == YNPageHeaderViewScaleModeTop) {
            if (yOffset < 0) {
                ...
                scaleBgViewFrame.size.width = self.config.pageViewWidth + fabs(xOffset) * 2;
            }
        } else {
            if (yOffset < 0) {
                headerBgViewFrame.origin.y = yOffset - _insetTop;

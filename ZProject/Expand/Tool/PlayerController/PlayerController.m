...
@property (nonatomic, strong) PlayerView *videoControl;
...
- (void)configControlAction
{
    ...
    [self.videoControl.collectButton addTarget:self action:@selector(collectButtonClick) forControlEvents:UIControlEventTouchUpInside];
...
{
    [self dismiss];
//    !self.dimissCompleteBlock ?: self.dimissCompleteBlock();
}

- (void)collectButtonClick
{
    self.collect = !self.collect;
    self.videoControl.isCollect = self.collect;
    !self.updateCollectBlock ?: self.updateCollectBlock(self.collect);
}
...
- (void)fullScreenButtonClick
{
    ...
    !self.hiddentStatusBarBlock ?: self.hiddentStatusBarBlock(YES);
    [UIView animateWithDuration:0.3f animations:^{
        ...
        self.videoControl.isFullscreenMode = YES;
...
- (void)shrinkScreenButtonClick
{
    ...
    !self.hiddentStatusBarBlock ?: self.hiddentStatusBarBlock(NO);
    [UIView animateWithDuration:0.3f animations:^{
        [self.view setTransform:CGAffineTransformIdentity];
        self.frame = self.originFrame;
    } completion:^(BOOL finished) {
        self.isFullscreenMode = NO;
        self.videoControl.isFullscreenMode = NO;
...
- (PlayerView *)videoControl
{
    if (!_videoControl) {
        _videoControl = [[PlayerView alloc] init];
    }
